...
class KG {
   public:
    ...
    void set_ent_sbert_embed(uint64_t, Eigen::VectorXd&);
    void clear_ent_embeds();
    Eigen::VectorXd& get_ent_embed(uint64_t);
    Eigen::VectorXd& get_ent_sbert_embed(uint64_t);
    ...
    std::unordered_map<uint64_t, Eigen::VectorXd> ent_sbert_emb_mp;
...
bool KG::is_sbert_embs_filled() {
    return !ent_sbert_emb_mp.empty();
}
...
void KG::set_ent_sbert_embed(uint64_t ent_id, Eigen::VectorXd& embeds) {
    ent_sbert_emb_mp[ent_id] = embeds;
}
...
Eigen::VectorXd& KG::get_ent_sbert_embed(uint64_t ent_id) {
    if (ent_sbert_emb_mp.count(ent_id)) {
        return ent_sbert_emb_mp[ent_id];
    }
    return EMPTY_EMB;
}
...
class EmbedEquiv {
   public:
    ...
    double get_sbert_eqv(uint64_t, uint64_t);
    bool has_emb_eqv();
    void init(uint64_t);
   ...
   private:
    ...
    double calculate_sbert_eqv(uint64_t, uint64_t);
};
...
double EmbedEquiv::calculate_sbert_eqv(uint64_t id, uint64_t cp_id) {
    Eigen::VectorXd& sbert_emb_a = kg_a->get_ent_sbert_embed(id);
    Eigen::VectorXd& sbert_emb_b = kg_b->get_ent_sbert_embed(cp_id);

    if (KG::is_emb_empty(sbert_emb_a) || KG::is_emb_empty(sbert_emb_b)) {
        // return 0.0;
        return -100.0;  // Set a very small number, then the literal mapping's prob will be 0
    }

    double eqv = sbert_emb_a.dot(sbert_emb_b) / (sbert_emb_a.norm() * sbert_emb_b.norm());
    return eqv;
}
...
double EmbedEquiv::get_sbert_eqv(uint64_t id, uint64_t cp_id) {
    double eqv = 0.0;
    if (cache_lock.try_lock()) {
        std::pair<uint64_t, uint64_t> cp_pair = std::make_pair(id, cp_id);
        if (cache.count(cp_pair)) {
            ...
            eqv = calculate_sbert_eqv(id, cp_id);
        }
        if (cache.size() < capacity) {
            cache[cp_pair] = eqv;
        }
...
void PARISEquiv::init_loaded_data(double threshold) {  // will be used in GCNAlign.py. This function is used to load the output of SE Module (in ent_eqv_mp) into ent_eqv_tuples, then initialize unaligned_ents
    std::vector<std::tuple<uint64_t, uint64_t, double>> new_ent_eqv_tuples;
    std::unordered_set<uint64_t> visited;
...
void PRModule::set_sbert_eqv_weight(double weight) {
    paris_params->SBERT_EQV_WEIGHT = weight;
}

void PRModule::set_emb_eqv_weight(double weight) {
    paris_params->EMB_EQV_WEIGHT = weight;
}
...
void PRModule::update_lite_eqv(uint64_t id_a, uint64_t id_b, double prob, bool forced) {  // will be used in kgs.init()
    if (forced) {
        insert_value_to_mp_mp(this->paris_eqv->get_forced_eqv_mp(), id_a, id_b, prob);
    } else {
        prob = get_filtered_prob(id_a, id_b, prob);
...
void PRModule::one_iteration_one_way_per_thread(PRModule* _this, std::queue<uint64_t>& ent_queue, KG* kg_l, KG* kg_r, bool ent_align) {
    ...
    std::function<void(std::unordered_map<uint64_t, double>&, uint64_t, double)> register_ongoing_rel_eqv_deno = [&](std::unordered_map<uint64_t, double>& mp, uint64_t cp_id, double prob) {
        if (!mp.count(cp_id)) {
            mp[cp_id] = 1.0;
        }
        mp[cp_id] *= (1.0 - prob);
    ...
    // register_ongoing_ent_eqv 只有在 kg_a 跑的時候才會執行
    ...
    while (!ent_queue.empty()) {
        ...
        std::function<void()> update_ent_eqv = [&]() {
            ...
            for (auto iter = ent_ongoing_eqv.begin(); iter != ent_ongoing_eqv.end(); ++iter) {
                ...
                while (!st1.empty() && st1.back().second < ent_cp_eqv) {  // if the prob of the last pair in st1 < the ongoing ent_cp_eqv
                    st2.emplace_back(st1.back());                         // save it in st2 in an ascending order
                    st1.pop_back();
                }

                if (st1.size() < _this->paris_params->ENT_CANDIDATE_NUM) {           // if st1 is not full
                    st1.emplace_back(std::make_pair(ent_cp_candidate, ent_cp_eqv));  // save the ongoing ent_cp_eqv in st1
                }

                while (st1.size() < _this->paris_params->ENT_CANDIDATE_NUM && !st2.empty()) {  // if st1 is not full and st2 is not empty
                    st1.emplace_back(st2.back());                                              // push it from st2 to st1 in a descending order
                    st2.pop_back();
                }
            }

            // put all pairs in st1 into ent_eqv_result, and an entity can have at most ENT_CANDIDATE_NUM pairs : (ent_cp, prob)
            while (!st1.empty()) {
...
void PRModule::one_iteration_one_way(std::queue<uint64_t>& q, KG* kg_l, KG* kg_r, bool ent_align) {
    ...
    for (int i = 0; i < thread_num; ++i) {
        threads.push_back(std::thread(&PRModule::one_iteration_one_way_per_thread, this, std::ref(q), kg_l, kg_r, ent_align));  // std::ref() is used to call by reference, which can let multiple threads to access the same queue and won't read repeatedly.
    }
